...
### Corrigido
- `README.md`: corrige contagem de testes nativos de 38/38 para 42/42 (badge e tabela "Estado do projeto") — ficou desatualizada quando a melhoria M1 adicionou 4 testes a `test_game`; contagem reconfirmada por execução real nesta sessão (`pio test -e native` — 42 succeeded, 2026-07-10)
...
### Adicionado
- `concept/escopo_e_limitacoes.md` (RASCUNHO v0.1.0) — posicionamento honesto do 42: o que o instrumento é, o que ele não é (não testado, não validado, não equivalente a instrumentos padronizados), como deve ser usado na prática (julgamento profissional, não protocolo fechado), responsabilidade sobre uso indevido, uma nota explícita de que versões anteriores deste projeto (histórico do git) citaram autores e teorias com uma confiança que não resistiu à auditoria — qualquer "certeza" científica encontrada em versão anterior deve ser tratada como não verificada até prova em contrário —, e o campo aberto para quem quiser pesquisar e validar de verdade no futuro. Derivado de uma pesquisa determinística sobre as referências científicas citadas em `00_conceito.md`, verificadas em texto bruto (Europe PMC / PubMed, sem resumo de terceiros)
- `.claude/IA_USO.md` — documento reflexivo, em linguagem acessível, sobre como a IA foi usada na elaboração deste projeto: origem no conceito discutido com IAs comerciais, o papel do `CLAUDE.md` como guia de sessão, e os limites reais impostos deliberadamente (bancada antes do commit, autorização antes do release, citações técnicas conferidas contra a fonte original). Fora da árvore verificada por `run_all.py` (diretório oculto `.claude/`), sem `depende_de`/`impacta`; referenciado a partir do `README.md`
...
### Alterado
- `concept/00_conceito.md` v0.4.0 → v0.5.0: §6.3 Martelos ganha registro explícito de que o martelo é opcional — como o sensor de impacto está na zona, não no martelo, bater diretamente com a mão também é detectado normalmente; §7.1/§7.2 (Modo 1/Modo 2) passam a citar a mão como alternativa ao martelo. Os nomes dos modos (`Modo 1 Martelo`, `Modo 2 Martelos`, enums `UM_MARTELO`/`DOIS_MARTELOS`) permanecem inalterados — são identificadores fixos, não uma exigência de uso do martelo
- `hardware/11_montagem.md` v0.3.5 → v0.4.0: Fase 9 (calibração do THRESHOLD) passa a exigir medição com martelo E com a mão — a batida com a mão tende a ser mais fraca, e o THRESHOLD precisa detectar as duas; §15.3 Martelos registra que o martelo é opcional para o uso, mas continua fazendo parte do exemplar físico
- `manual/12_manual_pedagogo.md` v0.3.0 → v0.4.0: descrição do instrumento, tabela de componentes e os dois modos passam a citar a mão como alternativa ao martelo
- `compliance/06_privacidade_lgpd.md` v0.3.0 → v0.4.0: descrição do instrumento em §3 passa a citar as duas formas de bater (martelo ou mão); nenhuma mudança na coleta, finalidade ou guarda dos dados
- Cascata mecânica do conceito v0.5.0 (bump PATCH, `depende_de`/Rastreabilidade, sem mudança de conteúdo — nenhum destes documentos trata do objeto usado para bater na zona): `system/01_arquitetura.md` v0.4.1→v0.4.2, `modules/sensor/02_sensor_impacto.md` v0.2.4→v0.2.5, `modules/visual/03_saida_visual.md` v0.1.9→v0.1.10, `modules/game/04_logica_jogo.md` v0.2.1→v0.2.2, `modules/power/05_alimentacao.md` v0.3.4→v0.3.5, `modules/interface/07_interface_pedagogo.md` v0.4.1→v0.4.2, `hardware/08_bom.md` v0.3.5→v0.3.6, `hardware/09_conexoes.md` v0.3.5→v0.3.6, `hardware/10_cablagem.md` v0.2.5→v0.2.6, `concept/escopo_e_limitacoes.md` v0.1.0→v0.1.1, `_governance/CODING_STANDARD.md` v0.2.6→v0.2.7, `_governance/TESTING_STANDARD.md` v0.1.8→v0.1.9, `_governance/WEB_STANDARD.md` v0.4.1→v0.4.2
- `README.md` — adiciona aviso em destaque logo após o título (antes dos badges), para quem não lê o resto do documento: instrumento não testado nem validado, qualquer referência citada no projeto deve ser tratada como não validada até prova em contrário, aponta para `concept/escopo_e_limitacoes.md`; remove o parágrafo equivalente que estava mais abaixo (redundante com o aviso novo)
- `compliance/06_privacidade_lgpd.md` v0.2.3 → v0.3.0 — mesma correção de honestidade aplicada ao documento que vai para os responsáveis legais (§`#o-que-e`): remove "instrumento de avaliação pedagógica com formato de brinquedo", adiciona parágrafo explícito para o consentimento do responsável (não testado, não validado, não substitui instrumentos padronizados, não é para diagnóstico formal); corrige tabela de Identificação, que mostrava versão desatualizada (0.2.2 em vez de 0.2.3)
- `.claude/IA_USO.md` — corrige afirmação factualmente incorreta: o documento dizia que toda citação científica havia sido conferida contra a fonte antes de entrar em documento aprovado; a auditoria desta sessão encontrou que isso não foi cumprido para 5 das 7 citações originais de `00_conceito.md`. Reescrito para admitir isso e tratá-lo como exemplo concreto do próprio ponto do documento (IA inventa citação com fluência; citação não conferida não é fundamentação)
- `concept/00_conceito.md` v0.3.0 → v0.4.0 — correção de honestidade derivada da pesquisa registrada em `concept/escopo_e_limitacoes.md`: das 7 referências científicas nomeadas em §13 (Fundamentos Pedagógicos e Técnicos), 5 não se sustentaram à conferência em texto bruto (Cowan 2001 mede adultos, não crianças; Swinnen 2002 e Corbetta & Thelen 1996 são sobre construtos/populações diferentes da janela de 800ms; Ruff & Lawson 1990 não mede o intervalo de 2000ms; Pellicano & Burr 2012 propõe o oposto do que a linha 8 afirmava) — essas linhas passam a dizer "Decisão de projeto" em vez de citar autor que não sustenta a afirmação; mantidas Wong (2011, paleta de cores) e Kail (1991, sem limite de tempo), únicas que resistiram à conferência; §3 (Contexto e Justificativa) reescrito — não superestima mais o instrumento como "avaliação" validada, reposicionado como instrumento lúdico de estímulo e registro, com disclaimer explícito de que não foi testado nem validado e de que os autores não se responsabilizam por uso indevido; §5.1 remove a atribuição incorreta a Cowan (2001); §10.3 e §9.5/§9.7 removem referências que não sustentavam os valores citados, mantendo `[CALIBRAR]`; nova §15 `#ideias-futuras` registra "Modo Memória Crescente" como ideia de modo adicional (não substitui Modo 1/2), ainda não especificado; `escopo_e_limitacoes.md` adicionado ao `impacta` [CONDICIONAL]
- `README.md` — remove o rótulo "instrumento de avaliação pedagógica" (§"O brinquedo"); adiciona parágrafo explícito de que o instrumento não foi testado nem validado cientificamente, não substitui instrumentos padronizados, não deve ser usado para diagnóstico clínico formal, e que os autores não se responsabilizam por uso indevido; adiciona `concept/escopo_e_limitacoes.md` à tabela de documentos
- `manual/12_manual_pedagogo.md` v0.2.0 → v0.3.0 — mesma correção de honestidade em `#o-que-e`: remove "instrumento de avaliação pedagógica", adiciona o mesmo parágrafo de limitação e responsabilidade
- Cascata mecânica do conceito v0.4.0 (bump PATCH/MINOR, `depende_de`/Identificação/Rastreabilidade, sem mudança de conteúdo funcional — correção de honestidade das referências científicas não afeta hardware/firmware/interface): `01_arquitetura.md` v0.4.0→v0.4.1, `05_alimentacao.md` v0.3.3→v0.3.4, `02_sensor_impacto.md` v0.2.3→v0.2.4, `03_saida_visual.md` v0.1.8→v0.1.9, `04_logica_jogo.md` v0.2.0→v0.2.1, `06_privacidade_lgpd.md` v0.2.2→v0.2.3, `07_interface_pedagogo.md` v0.4.0→v0.4.1, `08_bom.md` v0.3.4→v0.3.5, `09_conexoes.md` v0.3.4→v0.3.5, `10_cablagem.md` v0.2.4→v0.2.5, `11_montagem.md` v0.3.4→v0.3.5, `CODING_STANDARD.md` v0.2.5→v0.2.6, `TESTING_STANDARD.md` v0.1.7→v0.1.8, `WEB_STANDARD.md` v0.4.0→v0.4.1
- `.claude/CLAUDE.md` — ETAPA 1 (Orientação) ganha ponteiro para `.claude/IA_USO.md`; este é o ponto de entrada real (lido no início de toda sessão, junto com `SESSION_STATE.md` no retorno), não o `README.md`
- `README.md` — adiciona ponteiro para `.claude/IA_USO.md` na seção "Estado do projeto"
- `VALIDATION.md` — rodada de bancada 2026-07-10 (usuário): CA-01-07, CA-02-01, CA-02-02, CA-02-04, CA-07-11 e CA-07-UI-05 promovidos a PASSOU; correção de contagem da linha de estado (dizia 56 PASSOU · 11 PENDENTE; a tabela committed continha 58 · 9) — estado real 63 PASSOU · 1 OBSOLETO · 4 PENDENTE (+ UI 3/5). Registrados também: teste exploratório de brilho `setBrightness(255)` em firmware provisório (nada commitado em código) — funcionou em bancada, mas 255 excede a restrição `05_alimentacao.md#restricao-led` (brilho máximo 150); valor em vigor mantido em 150 por decisão do usuário, `[CALIBRAR]` de `03_saida_visual.md#cores-rgb` permanece aberto e calibração futura acima de 150 exige cascata iniciando em `05_alimentacao.md`; nota de conformidade de material da cablagem na seção CA-10 — bitolas/cores dos fios deste exemplar fora de `10_cablagem.md` §6/§7 por disponibilidade de material (decisão consciente do usuário, eletricamente validado), com a especificação permanecendo normativa para novos exemplares
